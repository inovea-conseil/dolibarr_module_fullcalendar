...
//	Get all Task
ob_start();
$select_element = $formProject->selectTasks(-1, 0, 'fk_element', 0, 0, 1, 1);
$select_element = ob_get_clean();
...
$(document).ready(function () {
	...
	$('#fullcalendar').fullCalendar({
		...
		eventRender: function (event, element, view) {
			...
			if(!empty($conf->global->FULLCALENDAR_SHOW_FK_ELEMENT)) {

			?>
			if (event.fk_element > 0) {
				element.append('<div style="z-index:3;position:relative;">' + event.task + '</div>');
				note = '<div style="z-index:3;position:relative;">' + event.task + '</div>' + note;
			}
			<?php
			}
			...
			if ($().tipTip) // ou $.fn.tipTip, mais $.tipTip ne fonctionne pas
			{
				...
				element.find(".classfortooltip").tipTip({
					maxWidth: "600px",
					edgeOffset: 10,
					delay: 50,
					fadeIn: 50,
				...
				element.find(".classforcustomtooltip").tipTip({
					maxWidth: "600px",
					edgeOffset: 10,
					delay: 50,
					fadeIn: 50,
	...
	function showPopIn(date, calEvent) {
		...
		if(!empty($conf->global->FULLCALENDAR_SHOW_FK_ELEMENT)){
		?>
		$form.append('<br /><?php echo $langs->trans('Task'); ?> : ');
		$form.append(<?php echo json_encode($select_element); ?>);
		<?php
		}
		...
		if (typeof calEvent === 'object') {
			...
			fk_element = calEvent.object.fk_element;
			...
			$div.find('#fk_element').val(calEvent.object.fk_element).trigger('change')
		...
		var formattedDateStart = '', formattedHoursStart = '', formattedMinutesStart = '', formattedDateEnd = '',
			formattedHoursEnd = '', formattedMinutesEnd = '';
		...
		var TButton = [];
		if (editable) {
			const saveElement = $('#fk_element').clone();
			setTimeout(() => {
				let numproject = (($('#select2-fk_project-container').html()).split(','))[0];
				$('#fk_element').children().each((i, obj) => {
					if (obj.dataset.html != null) {
						res = ((obj.dataset.html).split(" "))[0];
						console.log(res)
			...
			$('#fk_project').change(() => {
				//$.ajax({
				//	url: '<?php //echo dol_buildpath("fullcalendar/script/ajax.php", 1) ?>//',
				//	type: "POST",
				//	data: {fk_project: $("#fk_project").find('option:selected').val(), action: "searchTask"}
				//}).done(function (data) {
				//	console.log(data);
				//}).fail(function (data) {
				//	console.log("erreur")
				//});
				$('#fk_element').val(0);
				$('#select2-fk_element-container').html("");
			});
			...
			TButton.push({
					...
					, click: function () {
						...
						if ($('#pop-new-event input[name=label]').val() != '') {
							...
							console.log($('#pop-new-event #fk_element').val());
							...
							$.ajax({
								...
								, data: {
									...
									fk_user: TUserId
									,
									fk_project: <?php if (!empty($conf->global->FULLCALENDAR_SHOW_PROJECT)) { ?>$('#pop-new-event #fk_project').val()
									<?php } else { ?>fk_project<?php } ?>
									,
									fk_element: <?php if (!empty($conf->global->FULLCALENDAR_SHOW_FK_ELEMENT)) { ?>$('#pop-new-event #fk_element').val()
									<?php } else { ?>fk_element<?php } ?>
									,
