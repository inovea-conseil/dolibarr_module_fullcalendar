...
switch ($put) {
	...
	case 'event':
		...
		*/
		...
		$a->elementtype = (GETPOST('fk_element', 'int') == 0)?'':"task";
		$a->fk_element = GETPOST('fk_element', 'int');
		$a->elementid = GETPOST('fk_element', 'int');
		...
		if (empty($a->id)) {
			...
			if ($res > 0) {
				$res = $a->id;
				if (GETPOST('fk_element', 'int') == 0){
					$sql = "UPDATE llx_actioncomm SET elementtype = NULL,fk_element = NULL WHERE id = ".$a->id;
					$resql = $db->query($sql);
				}
...
//echo $sql;exit;
	...
	$TProjectTask = $TProjectObjecTask = array();
	$TEventObject = array();
	...
	foreach ($TEventObject as &$event) {
		...
		if (!empty($conf->global->FULLCALENDAR_SHOW_FK_ELEMENT)) {
			if ($event->elementtype == "task" && $event->fk_element != 0) {
				$t = new Task($db);
				$t->fetch($event->fk_element);
				$TProjectTask[$event->fk_element] = $t->getNomUrl(1);
				$TProjectObjectTask[$event->fk_element] = $t;
			}
		}
		...
		, 'task' => (!empty($TProjectTask[$event->fk_element]) ? $TProjectTask[$event->fk_element] : '')
	...
	$parameters = array('use_color_from' => GETPOST('use_color_from', 'none'), 'sql' => $sql);
	$action = 'getEvents';
